...
- [ ] a/an의 핵심 의미가 '하나'임을 이해했다
- [ ] a와 an 선택이 알파벳이 아닌 발음 기준임을 숙지했다
- [ ] "a university"와 "an hour"가 왜 그런지 설명할 수 있다
- [ ] 첫 언급 → a/an, 재언급 → the 패턴을 기억한다
...
- [ ] the의 핵심 의미가 '그'임을 이해했다
- [ ] the를 쓰는 3가지 조건(재언급, 공유 지식, 후치수식)을 나열할 수 있다
- [ ] 후치수식이 있으면 첫 언급에도 the를 쓴다는 것을 숙지했다
- [ ] a pen vs the pen의 뉘앙스 차이를 설명할 수 있다
...
```mermaid
flowchart TD
    A[명사를 말하려면?] --> B{셀 수 있는 명사?}
    B -->|예| C{단수? 복수?}
    B -->|아니오| D[무관사 가능 but some 권장]
    C -->|단수| E[반드시 한정사 필요]
    C -->|복수| F{일반화? 특정?}
    E --> G{처음 언급? 재언급?}
    G -->|처음| H[a/an 또는 소유격/지시사]
    G -->|재언급| I[the]
    G -->|후치수식| I
    F -->|일반화| J[무관사 복수 dogs]
    F -->|특정| K[the dogs / these dogs]
```
...
| [[YT-english-260305-전치사-접근법-기초-1편]] | 같은 채널 영상 | 전치사도 한정사와 마찬가지로 한국어에 없는 '범위 결정' 개념. 함께 학습 시 영어 구조 이해 심화 |
| [[YT-english-260310-수동태-완전정복]] | 같은 채널 영상 | 수동태에서도 a/the 선택이 중요. 이 노트의 규칙 적용 연습 |
...
**기존 지식과의 연결:**
- [[YT-english-260305-전치사-접근법-기초-1편]] — 전치사도 한국어에 없는 '공간/시간 범위 결정' 개념. 한정사와 전치사를 함께 이해하면 영어의 '명시적 범위 결정' 사고방식이 보인다.
- [[YT-english-260310-수동태-완전정복]] — 수동태 문장에서 a/the 선택 연습. "A book was written by him" vs "The book was written by him"의 차이.
