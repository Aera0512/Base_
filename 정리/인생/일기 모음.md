---
tags:
  - 모음집
  - 일기
---

## ✍️ 일기 모음

> [!info] 이 노트 정보
> 데일리 노트의 "자유롭게 → 오늘의 일기" 칸에 작성한 모든 내용을 날짜별로 모아서 보여줍니다.

---

```dataviewjs
const pages = dv.pages('"b. Dairy/1. Daily"')
  .sort(p => p.file.name, 'desc');

for (let page of pages) {
  const content = await dv.io.load(page.file.path);
  const lines = content.split('\n');

  let inDiary = false;
  let diaryContent = [];

  for (let i = 0; i < lines.length; i++) {
    const line = lines[i];

    // "오늘의 일기" 콜아웃 시작 감지
    if (line.includes('[!diary]') && line.includes('오늘의 일기')) {
      inDiary = true;
      continue;
    }

    // 콜아웃 내용 수집
    if (inDiary) {
      // 다른 콜아웃 시작 감지 (reflection 등)
      if (line.includes('[!') && !line.includes('[!diary]')) {
        break;
      }

      if (line.startsWith('>')) {
        const text = line.replace(/^>\s?/, '');
        diaryContent.push(text);
      } else if (line.trim() === '' && diaryContent.length > 0) {
        // 빈 줄이지만 아직 콜아웃 안
        continue;
      } else if (!line.startsWith('>')) {
        // 콜아웃 끝
        break;
      }
    }
  }

  // 내용이 있으면 출력
  const cleanContent = diaryContent.filter(l => l.trim().length > 0);
  if (cleanContent.length > 0) {
    dv.header(3, `📅 ${page.file.name}`);
    dv.paragraph(cleanContent.join('\n'));
    dv.paragraph('---');
  }
}
```
